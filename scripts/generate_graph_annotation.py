...
def generate_annotation(annotation, graph, node_out, edge_out):
    ...
    id2isol = dict([(re.sub(r'\.velvet', '', i), str(j)) for i, j in zip(
        g.graph["isolateNames"], range(len(g.graph["isolateNames"])))
                    ])  #resolve hzi isolate name clash
        ...
        for m in G.nodes[n]["members"]:
            ...
            for annot in annot_node_dicts:
                if name not in annot_node_dicts[annot]:
                    annot_node_dicts[annot][name] = set(
                        [annot_dicts[annot][m]])
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("annotation",
                        help='table with one annotation type per column')
    parser.add_argument("graph", help='final graph panaroo output')
    parser.add_argument("edge_out", help='edge output mapping file')
    parser.add_argument("node_out", help='node output mapping file')
