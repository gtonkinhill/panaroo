...
def add_to_queue(g, s, nodes, visited, sink, mapping, ref_g_id, max_dist):
    ...
    for i in nodes:
        ...
        if "%s_0_" % ref_g_id not in g.nodes[i]['geneIDs'] and i not in visited: 
            ...
            dist = get_dist(mapping.loc[g.nodes[s]['name'],"gene_id"], mapping.loc[g.nodes[i]['name'], "gene_id"], max_dist)
...
def remove_var_edges(g):
    for n in g:
        if g.nodes[n]["highVar"] == 1 and "%s_0_" % ref_g_id not in g.nodes[n]['geneIDs']:
            var_nodes.append(n)
    var_nodes = []
...
def layout(graph, ref_g_id, cut_edges_out, ignore_high_var, add_reference_edges):
    ...
    gene_order = [int(mapping.loc[n, "gene_id"].split("_")[2]) for n in mapping.index]
    ...
    while i < len(mapping.index):
        ...
        visited = set([nid])
        sink = {"sink": None}
        queue = add_to_queue(g, nid, g.neighbors(nid), visited, sink, mapping, ref_g_id, max_dist)
        #depth first search
        last_target = None
        while len(queue) != 0:
            #print(len(queue))
            ...
            queue = queue + add_to_queue(g, nid, neighbors, visited, sink, mapping, ref_g_id, max_dist)
        last_target = None
        ...
        if sink["sink"] is not None:
            ...
            for e in s_t_graph.edges:
                ...
                and "%s_0_" % ref_g_id in g.nodes[e[1]]['geneIDs']:
                    ...
                    if abs(int(n1.split("_")[2]) - int(n2.split("_")[2])) < 100:
                        #print(n1, n2)
                        remove.append(e)
            ...
            cut = []
            cut_weight, partitions = nx.algorithms.flow.minimum_cut(s_t_graph, nid, sink["sink"])
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser("enable reference-based layouting through detecting long-range connection between otherwise distant genes in a reference genome")
    parser.add_argument("ref_g_id", help='reference genome id (should be a complete genome)')
    parser.add_argument("graph", help='path to final_graph.gml')
    parser.add_argument("cut_edges_out", help='file for cut edges')
    parser.add_argument("--add_reference_edges", action = "store_true", help='add edges between consecutive genes in the reference genome even if they have been removed by panaroo')
    parser.add_argument("--ignore_high_var", action = "store_true",  help='ignore highly variable genes')
