...
def split_genome(fastafile, outputprefix, n_splits, n_reps):
    ...
    if len(seqs)>1: raise ValueError('Multiple contigs in fasta file which isnt currently supported!')
    h = ""
    s = seqs[0][1]
...
def main():
    ...
    parser.add_argument('-c', '--count', dest='n_splits', required=True,
                       type=int,
                       help='number of splits to make. Will result in n_splits + 1 fragments.')

    parser.add_argument('-r', '--rep', dest='n_reps', required=True,
                       type=int,
