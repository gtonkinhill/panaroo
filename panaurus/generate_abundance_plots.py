...
def get_curve_w_ci(pa_matrix, n_boot=100, method="chao2"):
    ...
    for n in range(start, n_samples + 1):
        ...
        if method=='acc':
            qm_boot = [pa_matrix[:, np.random.choice(n_samples, size=n, replace=True)] for i in range(1, n_boot + 1)]
        else:
            qm_boot = [
                get_q_m(pa_matrix[:, np.random.choice(n_samples, size=n, replace=True)])
        ...
        if method == "chao2":
            ...
            stat_boot = [
                jackknife(qm[0], qm[1], qm[2], 1) for qm in qm_boot
            ]
        elif method == "jack2":
            stat_boot = [
                jackknife(qm[0], qm[1], qm[2], 2) for qm in qm_boot
            ]
        else:
            stat_boot = [acc_curve(qm) for qm in qm_boot]

        stat_ci.append(np.quantile(stat_boot, [0.025, 0.5, 0.975]))
...
def jackknife(q, m, sobs, order=1):
    ...
    if order == 1:
        ...
        S = sobs + ((q[1] * (2 * m - 3)) / m -
                         ((q[2] * np.power(m - 2, 2)) / (m * (m - 1))))

    return S

def plot_quantiles(quantiles, sample_nums, method, outdir, color_shading='#1f78b4', color_mean='#1f78b4'):

    plt.style.use('ggplot')

    fig = plt.figure()

    plt.fill_between(sample_nums, [q[0] for q in quantiles], [q[2] for q in quantiles],
...
def generate_plot(pa_file, method, nboot, outdir):
    ...
    sample_nums, quantiles = get_curve_w_ci(pa_matrix, n_boot=nboot, method=method)
...
def get_options():
    ...
    io_opts.add_argument(
        ...
        dest="pa_file",
        required=True,
        help="gene presence absence file generated by Panaurus")
...
def main():
    ...
    if args.graph_type=='all':
        for method in ['chao2','ICE','jack1','jack2','acc']:
            generate_plot(args.pa_file, method, args.nboot,
                    args.output_dir)
    else:
