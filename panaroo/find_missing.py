...
#             # check that these two nodes are adjacent in the member contig
#             if member not in G.node[b[1]]['members']:
#                 idA = ""
#                 for seqid in G.node[b[0]]['seqIDs']:
#                     if seqid.split("_")[0] == member:
#                         idA = seqid.split("_")
#                         break
#                 idB = ""
#                 for seqid in G.node[b[2]]['seqIDs']:
#                     if seqid.split("_")[0] == member:
#                         idB = seqid.split("_")
#                         break

#                 if idA[1]==idB[1]:
#                     if abs(int(idA[2])-int(idB[2]))>1:
#                         continue
#                 else:
#                     edge_count = 0
#                     for neigh in G.neighbors(b[0]):
#                         if member in G[b[0]][neigh]['members']:
#                             edge_count+=1
#                     for neigh in G.neighbors(b[2]):
#                         if member in G[b[2]][neigh]['members']:
#                             edge_count+=1
