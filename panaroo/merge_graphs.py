...
def load_graphs(graph_files, n_cpu=1):
    ...
    graphs = Parallel(n_jobs=n_cpu)(
                delayed(nx.read_gml)(graph_file)
                for graph_file in tqdm(graph_files))

    return graphs


def cluster_centroids(graphs, outdir, identity_threshold=0.95, n_cpu=1):
    ...
    # Run cd-hit
    run_cdhit(temp_input_file.name, temp_output_file.name, 
...
def simple_merge_graphs(graphs, clusters):
    ...
    for cluster in clusters:
        ...
        non_conflicting_nodes = [n for n in cluster if graph_clust_count[n[0]]<2]
    ...
    for node in merged_G.nodes():
        ...
        for prev in reverse_mapping[node]:
            ...
            members += [str(prev[0]) + "_" + str(m) for m in graphs[prev[0]].node[prev[1]]['members']]
            centroid += ";".join([str(prev[0]) + "_" + str(m) for m in graphs[prev[0]].node[prev[1]]['centroid'].split(";")])
            seqIDs += [str(prev[0]) + "_" + d for d in graphs[prev[0]].node[prev[1]]['seqIDs']]
            ...
            for prev2 in reverse_mapping[edge[1]]:
                if prev1[0]==prev2[0]: #same graph
                    if graphs[prev1[0]].has_edge(prev1[1], prev2[1]):
                        ...
                        merged_G[edge[0]][edge[1]]['members'] += [str(prev1[0]
                            ) + "_" + str(m) for m in graphs[prev1[0]][prev1[1]][prev2[1]]['members']]

    return merged_G

# def generate_pairwise_aln_graph(graphs, merged_graph):
...
def main():
    ...
    for node in G.nodes():
        ...
        sub_graphs = list(set([m.split("_")[0] for m in G.node[node]['members']]))
    ...
    for edge in G.edges():
        ...
        sub_graphs = list(set([m.split("_")[0] for m in G.edges[edge[0], edge[1]]['members']]))
        G.edges[edge[0], edge[1]]['subGraphs'] = ";".join(conv_list(sub_graphs))
