...
def load_pa(presence_absence_file):
    ...
    pa_matrix = np.loadtxt(presence_absence_file, delimiter="\t", 
        skiprows=1, usecols=range(1,len(isolates)+1), dtype=int)

    return(isolates, genes, pa_matrix)

def f_getspectrum(pa_matrix):
    gene_sizes = np.bincount(np.sum(pa_matrix, 1), minlength=pa_matrix.shape[1]+1)
...
def f_meanpancore(Gk):
    ...
    for k in range(ngenomes):
        for n in range(ngenomes):
            if n <= ngenomes-k:
                Pabs[n,k] = np.prod((ngenomes-k-np.arange(1,n+2))/(ngenomes+1-np.arange(1,n+2)))
...
def f_pangenome(pa_matrix, nreps):
    ...
    for i in range(nreps):
        perm = np.random.choice(pa_matrix.shape[1], 
            size=pa_matrix.shape[1], replace=False)
...
def f_core(pa_matrix, nreps):
    ...
    for i in range(nreps):
        ...
        core_mat[:,i] = np.sum(cs==np.arange(1,pa_matrix.shape[1]+1), 
            0)
...
def f_coalescent(ngenomes, rho1, theta1, gess=None, rho2=None, theta2=None):
    ...
    for k in range(ngenomes):
        ...
        pangenome[k] = (theta1 * np.sum(1/(rho1-1+np.arange(1,k+2))) +
            theta2 * np.sum(1/(rho2-1+np.arange(1,k+2))) + gess)

        specprod1 = (k-np.arange(1, k+2)+2)/(k-np.arange(1,k+2)+rho1+1)
        specprod2 = (k-np.arange(1, k+2)+2)/(k-np.arange(1,k+2)+rho2+1)
...
def f_coalescent_spec(ngenomes, rho1, theta1, gess=None, rho2=None, theta2=None):
    ...
    specprod1 = (ngenomes-np.arange(1,ngenomes+1)+1)/(ngenomes-np.arange(1,ngenomes+1)+rho1)
    if  theta2>0:
        specprod2 = (ngenomes-np.arange(1,ngenomes+1)+1)/(ngenomes-np.arange(1,ngenomes+1)+rho2)
    else:
...
def get_tree_table(tree):
    ...
    for i, node in enumerate(tree.postorder_internal_node_iter()):
        ...
        tree_array[j][4] = len(node.leaf_nodes()) + len(list(node.postorder_internal_node_iter())) - 1
...
def f_fixed_spec(treetable, v1, u1, gess=None, v2=None, u2=None):
    ...
    nd = np.zeros(nn, dtype=int) #number of descendent nodes not counting itself
    ...
    for a in range(nn):
        ...
        if treetable[a][4]==0:
            ...
            #for k = 0
            gprob1[a,k] = ((treetable[desc_a, 5] + treetable[desc_a, 7] * gprob1[desc_a,k]) *
                (treetable[desc_b, 5] + treetable[desc_b, 7] * gprob1[desc_b,k]))
            gprob2[a,k] = ((treetable[desc_a, 6] + treetable[desc_a, 8] * gprob2[desc_a,k]) *
                (treetable[desc_b, 6] + treetable[desc_b, 8] * gprob2[desc_b,k]))
            ...
            for k in range(ndg[a]):
                ...
                gprob1[a,k+1] = (treetable[desc_a, 7] * treetable[desc_b, 5] * gprob1[desc_a,k+1] +
                    treetable[desc_b, 7] * treetable[desc_a, 5] * gprob1[desc_b,k+1] +
                    treetable[desc_a, 7] * treetable[desc_b, 7] * pre_sum)

                pre_sum = 0
                for j in range(0,k+2):
                    pre_sum += gprob2[desc_a,j]*gprob2[desc_b,k-j+1]
                gprob2[a,k+1] = (treetable[desc_a, 8] * treetable[desc_b, 6] * gprob2[desc_a,k+1] +
                    treetable[desc_b, 8] * treetable[desc_a, 6] * gprob2[desc_b,k+1] +
...
def f_theory_dist(params, data, constr, modeltype, fit, 
    genomesize=None, ng=None, treetable=None, return_theory=False):
...
def get_options():
    ...
    parser.add_argument(
        "--init_ess",
        dest="gess",
        help=("Initial value for the number of essential genes (default=1000)."),
        type=float,
        default=1000)
...
def main():
    ...
    if args.model=='coalescent':
        result = optimize.minimize(f_theory_dist,
            x0=params_init, method='Nelder-Mead',
            args=(data, (not args.no_constraint), args.model, args.fit, 
                genomesize, ngenomes),
            options=options)
    else:
        ...
        result = optimize.minimize(f_theory_dist,
            x0=params_init, method='Nelder-Mead',
            args=(data, (not args.no_constraint), args.model, args.fit, 
                genomesize, ngenomes, tree_table),
            options=options)
    ...
    pan, core = f_theory_dist(result['x'], data, (not args.no_constraint), args.model, "cp", 
        genomesize=genomesize, ng=ngenomes, treetable=tree_table, return_theory=True)

    with open(args.output_dir + "core_pangenome_size_estimate.csv", 'w') as outfile:
